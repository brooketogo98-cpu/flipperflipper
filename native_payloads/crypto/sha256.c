/*
 * SHA-256 Implementation - Minimal Size
 * For key derivation and checksums
 */

#include <stdint.h>
#include <string.h>
#include "sha256.h"

// SHA-256 constants
static const uint32_t K[64] = {
    0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5,
    0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
    0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3,
    0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
    0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc,
    0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
    0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7,
    0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
    0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13,
    0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
    0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3,
    0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
    0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5,
    0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
    0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208,
    0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2
};

// Rotate right
#define ROTR(x, n) (((x) >> (n)) | ((x) << (32 - (n))))

// SHA-256 functions
#define CH(x, y, z) (((x) & (y)) ^ (~(x) & (z)))
#define MAJ(x, y, z) (((x) & (y)) ^ ((x) & (z)) ^ ((y) & (z)))
#define SIG0(x) (ROTR(x, 2) ^ ROTR(x, 13) ^ ROTR(x, 22))
#define SIG1(x) (ROTR(x, 6) ^ ROTR(x, 11) ^ ROTR(x, 25))
#define sig0(x) (ROTR(x, 7) ^ ROTR(x, 18) ^ ((x) >> 3))
#define sig1(x) (ROTR(x, 17) ^ ROTR(x, 19) ^ ((x) >> 10))

// Use the header definition
#define sha256_ctx sha256_ctx_t

// Initialize SHA-256 context
void sha256_init(sha256_ctx* ctx) {
    ctx->state[0] = 0x6a09e667;
    ctx->state[1] = 0xbb67ae85;
    ctx->state[2] = 0x3c6ef372;
    ctx->state[3] = 0xa54ff53a;
    ctx->state[4] = 0x510e527f;
    ctx->state[5] = 0x9b05688c;
    ctx->state[6] = 0x1f83d9ab;
    ctx->state[7] = 0x5be0cd19;
    ctx->count = 0;
}

// Process one block
static void sha256_transform(sha256_ctx* ctx, const uint8_t data[64]) {
    uint32_t W[64];
    uint32_t a, b, c, d, e, f, g, h;
    uint32_t T1, T2;
    int i;
    
    // Prepare message schedule
    for (i = 0; i < 16; i++) {
        W[i] = ((uint32_t)data[i * 4] << 24) |
               ((uint32_t)data[i * 4 + 1] << 16) |
               ((uint32_t)data[i * 4 + 2] << 8) |
               ((uint32_t)data[i * 4 + 3]);
    }
    
    for (i = 16; i < 64; i++) {
        W[i] = sig1(W[i - 2]) + W[i - 7] + sig0(W[i - 15]) + W[i - 16];
    }
    
    // Initialize working variables
    a = ctx->state[0];
    b = ctx->state[1];
    c = ctx->state[2];
    d = ctx->state[3];
    e = ctx->state[4];
    f = ctx->state[5];
    g = ctx->state[6];
    h = ctx->state[7];
    
    // Main loop
    for (i = 0; i < 64; i++) {
        T1 = h + SIG1(e) + CH(e, f, g) + K[i] + W[i];
        T2 = SIG0(a) + MAJ(a, b, c);
        h = g;
        g = f;
        f = e;
        e = d + T1;
        d = c;
        c = b;
        b = a;
        a = T1 + T2;
    }
    
    // Add to state
    ctx->state[0] += a;
    ctx->state[1] += b;
    ctx->state[2] += c;
    ctx->state[3] += d;
    ctx->state[4] += e;
    ctx->state[5] += f;
    ctx->state[6] += g;
    ctx->state[7] += h;
    
    // Clear sensitive data
    for (i = 0; i < 64; i++) W[i] = 0;
}

// Update SHA-256 with data
void sha256_update(sha256_ctx* ctx, const uint8_t* data, size_t len) {
    size_t i;
    size_t index = (ctx->count >> 3) & 63;
    
    ctx->count += len << 3;
    
    // Handle partial block
    if (index) {
        size_t left = 64 - index;
        if (len < left) {
            memcpy(ctx->buffer + index, data, len);
            return;
        }
        memcpy(ctx->buffer + index, data, left);
        sha256_transform(ctx, ctx->buffer);
        data += left;
        len -= left;
    }
    
    // Process full blocks
    while (len >= 64) {
        sha256_transform(ctx, data);
        data += 64;
        len -= 64;
    }
    
    // Store remaining
    if (len) {
        memcpy(ctx->buffer, data, len);
    }
}

// Finalize SHA-256
void sha256_final(sha256_ctx* ctx, uint8_t hash[32]) {
    size_t index = (ctx->count >> 3) & 63;
    uint8_t padding[64];
    uint8_t length[8];
    int i;
    
    // Prepare padding
    memset(padding, 0, 64);
    padding[0] = 0x80;
    
    // Encode length
    for (i = 0; i < 8; i++) {
        length[i] = (ctx->count >> ((7 - i) * 8)) & 0xFF;
    }
    
    // Pad to 56 bytes
    if (index < 56) {
        sha256_update(ctx, padding, 56 - index);
    } else {
        sha256_update(ctx, padding, 64 - index + 56);
    }
    
    // Append length
    sha256_update(ctx, length, 8);
    
    // Output hash
    for (i = 0; i < 8; i++) {
        hash[i * 4] = (ctx->state[i] >> 24) & 0xFF;
        hash[i * 4 + 1] = (ctx->state[i] >> 16) & 0xFF;
        hash[i * 4 + 2] = (ctx->state[i] >> 8) & 0xFF;
        hash[i * 4 + 3] = ctx->state[i] & 0xFF;
    }
    
    // Clear context
    memset(ctx, 0, sizeof(sha256_ctx));
}

// Simple interface
void sha256(const uint8_t* data, size_t len, uint8_t hash[32]) {
    sha256_ctx ctx;
    sha256_init(&ctx);
    sha256_update(&ctx, data, len);
    sha256_final(&ctx, hash);
}

// PBKDF2-SHA256 for key derivation (simplified)
void pbkdf2_sha256(const uint8_t* password, size_t pass_len,
                   const uint8_t* salt, size_t salt_len,
                   uint8_t* key, size_t key_len, uint32_t iterations) {
    uint8_t U[32], T[32];
    uint8_t counter[4] = {0, 0, 0, 1};
    size_t key_pos = 0;
    
    while (key_pos < key_len) {
        // First iteration: HMAC(password, salt || counter)
        sha256_ctx ctx;
        uint8_t ipad[64], opad[64];
        uint8_t khash[32];
        
        // Prepare key
        if (pass_len > 64) {
            sha256(password, pass_len, khash);
            memset(ipad, 0x36, 64);
            memset(opad, 0x5c, 64);
            for (int i = 0; i < 32; i++) {
                ipad[i] ^= khash[i];
                opad[i] ^= khash[i];
            }
        } else {
            memset(ipad, 0x36, 64);
            memset(opad, 0x5c, 64);
            for (size_t i = 0; i < pass_len; i++) {
                ipad[i] ^= password[i];
                opad[i] ^= password[i];
            }
        }
        
        // Inner hash
        sha256_init(&ctx);
        sha256_update(&ctx, ipad, 64);
        sha256_update(&ctx, salt, salt_len);
        sha256_update(&ctx, counter, 4);
        sha256_final(&ctx, U);
        
        // Outer hash
        sha256_init(&ctx);
        sha256_update(&ctx, opad, 64);
        sha256_update(&ctx, U, 32);
        sha256_final(&ctx, U);
        
        // Copy to T
        memcpy(T, U, 32);
        
        // Additional iterations
        for (uint32_t i = 1; i < iterations; i++) {
            // HMAC of previous U
            sha256_init(&ctx);
            sha256_update(&ctx, ipad, 64);
            sha256_update(&ctx, U, 32);
            sha256_final(&ctx, U);
            
            sha256_init(&ctx);
            sha256_update(&ctx, opad, 64);
            sha256_update(&ctx, U, 32);
            sha256_final(&ctx, U);
            
            // XOR with T
            for (int j = 0; j < 32; j++) {
                T[j] ^= U[j];
            }
        }
        
        // Copy to output
        size_t copy_len = (key_len - key_pos) < 32 ? (key_len - key_pos) : 32;
        memcpy(key + key_pos, T, copy_len);
        key_pos += copy_len;
        
        // Increment counter
        counter[3]++;
    }
    
    // Clear sensitive data
    memset(U, 0, 32);
    memset(T, 0, 32);
}